...
/**
 * @brief Timeout slider constructor
 * @param parent Parent widget (optional)
 *
 * Creates a slider widget used to select a timeout duration of
 * several options.
 */
QTimeoutSlider::QTimeoutSlider (QWidget * parent):
    QWidget (parent)
{
...
/**
 * @brief Check whether the slider is valid.
 * @return True if the slider has at least value option and some
 * option is selected. Otherwise, returns false.
 */
bool QTimeoutSlider::valid (void) const
{
...
/**
 * @brief Currently set timeout value
 * @return The currently selected timeout value in seconds.
 */
unsigned int QTimeoutSlider::value (void) const
{
    if (p_Values.count () && p_Current >= 0) {
...
unsigned int QTimeoutSlider::maximum (void) const
{
    if (p_Values.empty ()) {
	    return (Never);
    } else {
...
unsigned int QTimeoutSlider::minimum (void) const
{
    if (p_Values.empty ()) {
	    return (Never);
    } else {
...
bool QTimeoutSlider::fixedWidthNumbers (void) const
{
	return (p_FixedWidthNumbers);
}

QString QTimeoutSlider::text (void) const
{
    return (p_Label->text ());
}

QString QTimeoutSlider::formatString (void) const
{
	return (p_FormatString);
}

QString QTimeoutSlider::neverString (void) const
{
	return (p_NeverString);
}

void QTimeoutSlider::setFormatString (const QString & str)
{
	p_FormatString = str;
...
void QTimeoutSlider::setNeverString ( const QString & str )
{
	p_NeverString = str;
...
void QTimeoutSlider::addValue ( unsigned int nsec )
{
...
void QTimeoutSlider::setValue (unsigned int nsec)
{
	int	i, n = p_Values.count ();
...
void QTimeoutSlider::delValue (unsigned int nsec)
{
...
void QTimeoutSlider::setFixedWidthNumbers (bool fixed)
{
...
void QTimeoutSlider::slotValueChanged (int val)
{
	p_Current = val;
...
void QTimeoutSlider::slotUpdateLabel (void)
{
